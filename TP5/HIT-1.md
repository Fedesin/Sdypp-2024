...
- Visite https://medium.com/trabe/a-brief-introduction-to-webgl-5b584db3d6d6 y agregue a su informe información sobre el pipeline de renderizado. En particular busque relacionarlo con el artículo anterior.
  ...
  En el artículo anterior, se mencionaban los diferentes tipos de shaders y el objetivo final de cada uno. Las etapas del pipeline incluyen el Vertex Shader y el Fragment Shader, que son programables y permiten aplicar 
  transformaciones y efectos a los gráficos. 
...
- Diríjase a https://www.shadertoy.com/howto y agregue a su informe un listado de las salidas posibles de los Pixel Shaders así como su tipo y una breve descripción de qué representa cada una.
  ...
  a. **¿Que representa uv?**
     ...
     El array uv representa las coordenadas de la pantalla de forma normalizada. Basicamente, es un vector que contiene las coordenadas normalizadas del pixel actual en la pantalla, es decir, transforma la posición xy de un pixel en una posición uv que se podrá ubicar desde (0,0) en la esquina inferior izquierda hasta (1,1) en la esquina superior derecha de la pantalla.
  
   Por ejemplo, Shadertoy tiene el display donde muestra los resultados en un sistema de coordenadas que va de por 742x417. Este espacio lo vamos a transformar a un sistema de coordenadas uv que va 0 a 1.

  b. **¿Por qué es necesario trabajar en uv y no en xy?**
  
     Se trabaja con coordenadas normalizadas en lugar de coordenadas de píxel xy para garantizar que el shader funcione correctamente en diferentes resoluciones de pantalla. La GPU en un principio no sabe la resolución en la que estamos trabajando, por lo cuál, transformando xy en uv, evitamos lidear con el aspect ratio.

  Es decir, se trata a todas las resoluciones por igual, lo cuál significa que el mismo shader producirá los mismos efectos sin importar la resolución. Esto no ocurriría si se trabajara con las coordenadas xy, ya que probablemente el shader se comportaría de manera variante dependiendo del tamaño de la pantalla.
